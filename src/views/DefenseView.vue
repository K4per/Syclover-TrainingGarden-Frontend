...
<template>
  <section v-if="challenge">
    ...
    <div class="two-column challenge-layout">
      ...
      <aside class="panel submit-panel"><p class="eyebrow">PATCH &amp; AUTO CHECK</p><h2>防御补丁</h2><p class="muted">提交 patch.zip。Pwn 包含修改后的二进制和 fix.sh；Web 包含 fix.sh 以及需要覆盖的全部源码和资源。部署时平台会把文件放入 /app 并执行 fix.sh。</p>
        <label class="drop-zone"><input type="file" accept=".zip,application/zip" @change="patchFile = $event.target.files[0]"><span>＋</span><b>{{ patchFile?.name || '选择 patch.zip' }}</b><small>仅支持 .zip，必须包含根目录 fix.sh</small></label><button class="primary wide" :disabled="busy || !patchFile" @click="uploadPatch">上传并校验</button>
